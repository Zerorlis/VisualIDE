...
var workarea = new Vue({
    ...
    "methods": {
        ...
        levelRight(event) {
            event = event || window.event;
            if (commonFunction.getLocFromDom(event.currentTarget)["level"] == 0) {
                this.showRM(event, 'levelrmInLevel0');
            } else {
        ...
        jsReadFiles() {
            ...
            if (files.length) {
                ...
                reader.onload = function () {
                    let flow = null;
                    try {
                        flow = toCompute_flow(this.result);
                    } catch (e) {
                        alert("数据流图文件有一些错误，见浏览器控制台得到更多的消息");
                        return;
                    }
                    if (flow) {
                        workarea.computer_flow = flow["flow"];
                        workarea.name = flow["name"];
                        setTimeout(line.drawAllLine, 1);
                    }
                }
        ...
        save() {
            ...
            try {
                fileContent = toCfiFile(this.computer_flow, this.name);
            } catch (e) {
                throw e;
                alert("数据流图有一些错误，见浏览器控制台得到更多的消息");
                return;
            }
            this.download(this.name + ".cfl", fileContent);
        },
        // 把当前的图给送到服务器端，执行，并且更新后面的结果
        execFlow() {
            let fileContent = null;
            lastVariableLevel = this.getLastLevelVariable()
            if (lastVariableLevel.length != 1) {
                alert("最后一层必须只有一个输出变量"); // 平时做保存的时候啊还可以存着，但是如果执行了，要做一些检测，比如最后只能有一个输出
                return;
            }
            try {
                fileContent = toCfiFile(this.computer_flow, this.name);
            ...
            let filename = this.name + ".cfl";
            var output = [];
            function outputFunction() {
                console.log("template 结果:");
                console.log(output);
                if (output.length == 1) {
                    output[0] = output[0].replace(/(^\n*)|(\n*$)/gm, "");
                }
                if (output.length == 1 && output[0] != "__error__") {
                    lastVariableLevel[0].value = output[0];
                    alert("运行成功，结果是" + output[0])
                } else {
                    alert("运行时错误，服务器返回__error__");
                }
            }
            getFromWS("examples/api/execcomputeflow.template", "filename=" + filename + "$^@^$file_content=" + fileContent + "$^@^$parameter=" + this.parameter, output, outputFunction);
...
function toCfiFile(flow, flowName) {
    ...
    function processRuleLevel(ruleLevel, levelIndex, lastVariableLevel) {
        ...
        for (let ruleIndex = 0; ruleIndex < ruleLevel.length; ruleIndex++) {
            ...
            ruleString = ruleString.replace("斚斖斴", arulestr);
        }
        ruleString = ruleString.replace("斚斖斴", "");
        return ruleString;
    }
    function processVariableLevel(variableLevel, levelIndex, lastRuleLevel) {
        ...
        for (let variableIndex = 0; variableIndex < variableLevel.length; variableIndex++) {
            ...
            variableString = variableString.replace("斚斖斴", avariablestr);
        }
        variableString = variableString.replace("斚斖斴", "");
    ...
    for (let levelIndex = 1; levelIndex < flow.length; levelIndex++) {
        ...
        computer_flowStr = computer_flowStr.replace("斚斖斴", levelStr);
    }
    computer_flowStr = computer_flowStr.replace("斚斖斴", "");
    return computer_flowStr;
}

window.onbeforeunload = function (e) {
    (e || window.event).returnValue = '确定离开此页吗？会导致当前的数据流图消失，请确定保存后退出';
}
